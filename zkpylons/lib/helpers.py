...
def slideshow(fileset, small=None):
    """
    ...
    try:
        if small == None or small == "":
            ...
            # the width of image.
            small = random.choice(glob(file_paths["public_path"] + "/images/" + fileset + "/small/*"))
        else:
            small = file_paths["public_path"] + "/images/" + fileset + "/small/" + small

        output = "<div class=\"slideshow\" id=\"%s\" style=\"width: %dpx\">" % (fileset, int(Image.open(small).size[0]))
        small = os.path.basename(small)

        # Optionally load up some captions for the images.
        caption = dict()
        caption_file = file_paths['public_path'] + "/images/" + fileset + "/captions"
        ...
        # Load up all the images in the fileset directory.
        files = glob(file_paths['public_path'] + "/images/" + fileset + '/*')
        for file in files:
            if os.path.isfile(file):
                ...
                output += "<a href=\"" + file_paths["public_html"] + "/images/" + fileset + "/" + short_file + "\" rel=\"lightbox[" + fileset + "]\""
                ...
                # it as well.
                if short_file == small:
                    output += "<img src=\"" +  file_paths["public_html"] + "/images/" + fileset + "/small/" + short_file + "\">"
